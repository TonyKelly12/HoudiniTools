    ...
    def get_weapon_parts(self, weapon_type, part_type, page=1, limit=10):
        ...
        try:
            ...
            url = (
                ...
                f"weapon_type={weapon_type}&part_type={part_type}"
    ...
    def assemble_weapon_parts(self, model_files, assembly_data):
        ...
        try:
            ...
            for part_type, model_path in model_files.items():
                ...
                xform_node.parmTuple("t").set(
                    [position.get("x", 0), position.get("y", 0), position.get("z", 0)]
                )
                xform_node.parmTuple("r").set(
                    [rotation.get("x", 0), rotation.get("y", 0), rotation.get("z", 0)]
                )
                xform_node.parmTuple("s").set(
...
def onNodeCreated():
    ...
    if not node.parm("api_url"):
        ...
        output_path_parm = hou.StringParmTemplate(
            "output_path", "Output Path", 1, default_value=["$HIP/geo/weapons"]
        )
