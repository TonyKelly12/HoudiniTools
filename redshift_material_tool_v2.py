...
class RedshiftMaterialTool:
    ...
    def scan_textures(self):
        ...
        for root, dirs, files in os.walk(self.tex_path):
            ...
            texture_files = [f for f in files if any(f.lower().endswith(ext) for ext in self.texture_extensions)]
            ...
            if rel_path == '.':
                ...
                mesh_name = parts[0] if parts and parts[0] != '.' else os.path.basename(root)
            ...
            for file_or_group in file_groups:
                ...
                print(f"Added UDIM texture {texture_type} to material {material_name} (mesh: {mesh_name})")
            ...
            for file_or_group in file_groups:
                ...
                print(f"Added regular texture {texture_type} to material {material_name} (mesh: {mesh_name})")
    ...
    def _extract_material_name(self, filename):
        ...
        extra_keywords = ["Color", "DisplaceHeightField", "EmissionColor", "Metalness", 
                         "Normal", "Roughness", "alpha", "translucency", "AO", "height"]
    ...
    def _identify_texture_type(self, filename):
        ...
        if "alpha" in clean_name:
            print(f"  Identified as: alpha texture")
            return "alpha"
            
        if "translucency" in clean_name or "translucent" in clean_name:
            print(f"  Identified as: translucency texture")
            return "translucency"
        ...
        print(f"  No specific type identified, defaulting to: basecolor")
        return "basecolor"
        ...
        try:
                ...
                try:
                    ...
                    try:
                        if tex_type == "basecolor":
                            ...
                            print(f"Connected translucency texture")
                        elif tex_type == "alpha":
            ...
            if "normal" in textures or "bump" in textures:
                try:
                    ...
                    print(f"Connected bump node to material")
                except Exception as e:
...
class RedshiftMaterialToolDialog(QtWidgets.QDialog):
    ...
    def create_ui(self):
        ...
        self.create_folders_checkbox = QtWidgets.QCheckBox("Create separate folder for each material")
        ...
        if os.path.exists(expanded_path):
            ...
            if not material_sets:
                QtWidgets.QMessageBox.warning(
                    self, "No Textures Found", 
                    f"No texture sets found in:\n{expanded_path}\n\n"
                    f"Make sure your textures follow the expected naming conventions and folder structure."
                )
                return
        
            # Post-process to consolidate materials by base name
            consolidated_materials = {}
            
            # First pass - consolidate by extracting clean base names
            for mesh_name, materials in material_sets.items():
                consolidated_materials[mesh_name] = {}
                
                # Group all textures by their base material name
                for material_name, textures in materials.items():
                    # Get the base material name without the texture type suffix
                    base_material_name = self.extract_base_material_name(material_name)
                    
                    # If this base name doesn't exist yet, create it
                    if base_material_name not in consolidated_materials[mesh_name]:
                        consolidated_materials[mesh_name][base_material_name] = {}
                    
                    # Add all textures to this consolidated material
                    for tex_type, tex_info in textures.items():
                        consolidated_materials[mesh_name][base_material_name][tex_type] = tex_info
            
            # Debug - print the consolidated materials
            print("=== CONSOLIDATED MATERIALS ===")
            for mesh_name, materials in consolidated_materials.items():
                print(f"Mesh '{mesh_name}': {len(materials)} materials")
                for material_name, textures in materials.items():
                    print(f"  Material '{material_name}': {len(textures)} textures")
                    for tex_type in textures.keys():
                        print(f"    - {tex_type}")
            
            # Add the consolidated materials to the list widget
            for mesh_name, materials in consolidated_materials.items():
                for material_name, textures in materials.items():
    ...
    def extract_base_material_name(self, material_name):
        ...
        for suffix in ["Color", "DisplaceHeightField", "EmissionColor", "Metalness", 
                      "Normal", "Roughness", "alpha", "translucency", "AO", "height",
                      "basecolor", "diffuse", "albedo", "roughness", "rough", "metallic",
                      "metal", "normal", "bump", "displacement", "emission", "ao"]:
            
            # Check for suffix at end with dot or underscore
            if re.search(rf"[_\.]({re.escape(suffix)})$", clean_name, re.IGNORECASE):
                clean_name = re.sub(rf"[_\.]({re.escape(suffix)})$", "", clean_name, flags=re.IGNORECASE)
                break
            
            # Check for suffix in middle with separators
            if re.search(rf"[_\.]({re.escape(suffix)})[_\.]", clean_name, re.IGNORECASE):
                clean_name = re.sub(rf"[_\.]({re.escape(suffix)})[_\.]", "", clean_name, flags=re.IGNORECASE)
                break
    ...
    def create_materials(self):
        """Create the materials based on the selected items in the list"""
        # Check if any materials are selected
        if self.material_list.count() == 0:
            QtWidgets.QMessageBox.warning(
                self, "No Materials", "No materials found. Please scan for textures first.")
            return
        
        # Update the options based on checkboxes
        overwrite_existing = self.overwrite_checkbox.isChecked()
        create_folders = self.create_folders_checkbox.isChecked()
        ...
        # Process selected materials or all if none selected
        selected_items = self.material_list.selectedItems()
        if not selected_items:
            # No selection, process all
            material_items = [self.material_list.item(i) for i in range(self.material_list.count())]
        else:
        ...
        progress = QtWidgets.QProgressDialog(
            "Creating materials...", "Cancel", 0, len(material_items), self)
        ...
        for i, item in enumerate(material_items):
            ...
            material_data = item.data(QtCore.Qt.UserRole)
            material_name = material_data["material_name"]
            textures = material_data["textures"]
            
            # Check if material already exists
            existing_mat = self.material_tool.check_material_exists(mat_context, material_name)
            
            if existing_mat and not overwrite_existing:
                progress.setLabelText(f"Skipping existing material: {material_name}")
                skipped_count += 1
                continue
            
            # Delete existing material if we're overwriting
            if existing_mat and overwrite_existing:
            ...
            try:
                ...
                if create_folders:
                    ...
                    material_folder = mat_context.createNode("subnet", f"FOLDER_{material_name}")
        ...
        progress.setValue(len(material_items))
